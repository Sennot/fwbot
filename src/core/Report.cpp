...
std::string reportJSON(Replay const&r,Config const&c,std::vector<Row> const&rows,std::string const&status,std::string const&fingerprint,Tick actualEnd){
    json out={{"schema","frame-window-lab/1"},{"status",status},{"levelFingerprint",fingerprint},
        {"game","2.2081"},{"geodeSDK","5.8.2"},{"physics","240 TPS; native whole-step input; no CBF or corrections"},
        {"windowMeaning","Conditional on fixed other inputs, fixed seed and the selected endpoint"},
        ...
        {"settings",{{"firstInput",c.first+1},{"lastInput",c.last+1},{"radius",c.radius},{"endTick",c.endTick},{"actualEndTick",actualEnd},{"frameOffset",c.frameOffset},{"repeats",c.repeats},{"baselineRepeats",c.baselineRepeats},{"mode",c.mode==ScanMode::Edge?"edge":"hold-pair"}}}};
    out["rows"]=json::array();
    for(auto const&row:rows){
        ...
        for(auto const&p:row.probes)j["probes"].push_back({{"offset",p.offset},{"result",verdictName(p.verdict)},{"stoppedAt",p.stoppedAt}});
...
std::string reportCSV(Replay const&r,Config const&c,std::vector<Row> const&rows){
    std::ostringstream s;s<<"input,frame,time_seconds,player,button,edge,mode,paired_input,window_at_original,total_passing_ticks,offset_intervals,complete,left_search_limit,right_search_limit,left_sequence_limit,right_sequence_limit,removal\n";
